...
const EXAMPLE_MARKER_FILES = new Map([
  ...
  ['server/src/database/mod.rs', 1],
  ['server/src/error.rs', 2],
  ['server/src/service/mod.rs', 1],
  ['front/src/App.vue', 1],
...
const EXAMPLE_RESET_FILES = new Map([
  [
...
const EXAMPLE_FILES = [
  ...
  'front/src/example.css',
  'front/src/components/ExampleResources.vue',
...
function requireNoLocalArtifacts(root) {
  const present = BLOCKING_LOCAL_PATHS.filter((path) =>
...
function readTemplateIdentity(root) {
  ...
  if (!existsSync(path)) {
    fail(
      `${STATE_PATH} is missing; this repository is not an initializable template`,
    )
  }
  ...
  for (const field of requiredIdentityFields) {
    if (
      typeof manifest.identity?.[field] !== 'string' ||
...
export function deriveIdentity(
  ...
  displayName,
  githubRepository,
  projectDescription = defaultProjectDescription(displayName),
) {
  return {
    projectSlug,
...
export function defaultProjectDescription(displayName) {
  return `${displayName} is a backend-first Rust and Vue application.`
}
...
export function validateProjectSlug(projectSlug, sourceIdentity = undefined) {
  ...
  if (!SLUG_PATTERN.test(projectSlug)) {
    fail(
      'project slug must be lowercase ASCII kebab-case and start with a letter',
    )
  }
  if (projectSlug.length > 58) {
    fail(
      'project slug must be at most 58 characters so the PostgreSQL test database name fits',
    )
  }
...
export function validateDisplayName(displayName, sourceIdentity = undefined) {
  ...
  if (
    sourceIdentity &&
    sourceTokens(sourceIdentity).some((token) => displayName.includes(token))
  ) {
...
export function validateProjectDescription(
  ...
  if (projectDescription !== projectDescription.trim()) {
    fail('project description must not have leading or trailing whitespace')
  }
  const characters = Array.from(projectDescription)
  if (characters.length < 1 || characters.length > 200) {
    fail('project description must contain between 1 and 200 characters')
  }
  if (/\p{Cc}/u.test(projectDescription)) {
    fail('project description must not contain newlines or control characters')
  }
  if (
    [TEMPLATE_MARKER, EXAMPLE_MARKER].some((marker) =>
      projectDescription.includes(`${marker}:`),
    )
  ) {
    fail('project description must not contain template lifecycle markers')
  }
  if (
    sourceIdentity &&
    sourceTokens(sourceIdentity).some((token) =>
      projectDescription.includes(token),
    )
  ) {
    fail('project description must not contain a template source identifier')
  }
}
...
export function parseGitHubRepository(value) {
  ...
  if (parts.length !== 2) {
    fail(
      'GitHub repository must be owner/repository or a standard github.com URL',
    )
  }
...
function sourceTokens(identity) {
  return [
    identity.frontendPackage,
    identity.projectSlug,
    identity.rustIdentifier,
  ]
}
...
function validateTargetIdentity(target, source) {
  ...
  validateProjectDescription(target.projectDescription, source)
  ...
  if (
    sourceTokens(source).some((token) => canonicalRepository.includes(token))
  ) {
...
function replaceIdentity(text, source, target) {
  ...
  const patterns = [...replacements.keys()].sort(
...
function replaceExact(text, from, to, path, expectedCount = 1) {
  ...
  if (count !== expectedCount) {
    fail(
      `${path} expected ${expectedCount} occurrence(s) of a required template value; found ${count}`,
    )
  }
...
function transformMarkers(tree, marker, expectedFiles) {
  for (const [path, expectedCount] of expectedFiles) {
    ...
    if (transformed.count !== expectedCount) {
      fail(
        `${path} expected ${expectedCount} ${marker} block(s); found ${transformed.count}`,
      )
    }
...
function resetFiles(tree, files) {
  for (const [path, content] of files) {
    const entry = tree.get(path)
    if (!entry) {
      fail(`${path} is required to reset example-only content`)
    }
    entry.buffer = Buffer.from(content)
  }
}
...
function planInitialization(snapshot, sourceIdentity, target) {
  ...
  requireFiles(
    snapshot,
    EXAMPLE_RESET_FILES.keys(),
    'reset example-only content',
  )
  ...
  resetFiles(tree, EXAMPLE_RESET_FILES)
...
function validateWrittenProject(root, expectedIdentity) {
  ...
  if (
    ...
    fail(
      'server/Cargo.toml package, default-run, and binary names are inconsistent',
    )
  }
  ...
  const frontendPackage = JSON.parse(
    readFileSync(join(root, 'front/package.json'), 'utf8'),
  )
  const frontendLock = JSON.parse(
    readFileSync(join(root, 'front/package-lock.json'), 'utf8'),
  )
  if (
...
function printSummary(sourceIdentity, target, runCheck, root) {
  ...
  console.log(`  description:        ${target.projectDescription}`)
  ...
  if (basename(root) !== target.projectSlug) {
    console.log(
      `  warning: directory '${basename(root)}' does not match the project slug`,
    )
  }
  if (
    detectedGitHubRepository(root, sourceIdentity) !== target.githubRepository
  ) {
    console.log(
      '  warning: the selected GitHub repository does not match the detected origin',
    )
  }
}
...
function validateAnswersObject(answers) {
  ...
  const unknown = Object.keys(answers)
    .filter((key) => !supported.has(key))
    .sort()
  if (unknown.length) {
    fail(
      `non-interactive answers contain unsupported field(s): ${unknown.join(', ')}`,
    )
  }
...
async function collectInitAnswers(args, root, sourceIdentity) {
  ...
  if (answersFile) {
    ...
    const projectDescription =
      ...
      defaultProjectDescription(answers.displayName)
    return {
  ...
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    fail(
      'interactive initialization requires a TTY; automation must use --answers-file',
    )
  }
  ...
  try {
    ...
    const projectDescription = await askText(
      interface_,
      'Project description',
      defaultProjectDescription(displayName),
    )
    validateProjectDescription(projectDescription, sourceIdentity)
    ...
    const runCheck = await askYesNo(
      interface_,
      'Run just check after initialization?',
      true,
    )
...
export async function runInitCli(args, root = PROJECT_ROOT) {
  ...
  console.log(
    `Initialized '${target.displayName}' across ${changes.length} tracked files.`,
  )
  console.log(
    'Example resources, migrations, and template tooling were removed.',
  )
