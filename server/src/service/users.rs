...
pub async fn list(pool: &DbPool) -> HttpResult<Vec<User>> {
    database::users::list(pool).await.map_err(HttpError::from)
}

pub async fn get(pool: &DbPool, user_id: i64) -> HttpResult<Option<User>> {
    database::users::get(pool, user_id)
        .await
        .map_err(HttpError::from)
}

pub async fn delete(pool: &DbPool, user_id: i64) -> HttpResult<bool> {
