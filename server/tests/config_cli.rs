...
#[test]
fn strict_config_check_reports_a_safe_postgres_summary() {
    let temporary_directory = tempdir().expect("temporary directory should be created");
    let secret = "unique-cli-secret-marker";
    let output = application_command()
        .args(["config", "check", "--strict", "--format", "json"])
...
#[test]
fn unknown_environment_keys_warn_normally_and_fail_strictly_without_values() {
    let temporary_directory = tempdir().expect("temporary directory should be created");
    ...
    let strict = application_command()
        .args(["config", "check", "--strict"])
        .env("APP_DATA_DIR", temporary_directory.path())
        .env("APP_MISSPELLED_SETTING", secret)
        .output()
        .expect("strict check should run");
    assert!(!strict.status.success());
    let strict_stderr = String::from_utf8_lossy(&strict.stderr);
    assert!(strict_stderr.contains("APP_MISSPELLED_SETTING"));
    assert!(!strict_stderr.contains(secret));
}
